...
# 선형 영역 설정 (필요하면 조정)
linear_region = (Vg > 0.5) & (Vg < 10)  # 문턱 전압 근처 영역
...
Vth = -intercept / slope  # Threshold Voltage 계산

# 2. Subthreshold Swing (SS) 계산
log_Id = np.log10(Id)  # 드레인 전류 로그 변환

# Subthreshold 영역 설정 (Vg 값 조정 필요)
subthreshold_region = (Vg > 0) & (Vg < 3)  # 필요한 범위로 수정
...
# 그래프 (Vth, SS, Mobility)
plt.figure()
plt.plot(Vg, sqrt_Id, 'o', label='Measured sqrt(Id)')
plt.plot(Vg_linear, slope * Vg_linear + intercept, 'r-', label=f'Fit: Vth={Vth:.2f} V')
plt.xlabel('Vg (V)')
plt.ylabel('sqrt(Id)')
...
plt.plot(Vg_sub, sub_slope * Vg_sub + intercept, 'r-', label=f'Fit: SS={SS:.2f} V/dec')
plt.xlabel('Vg (V)')
plt.ylabel('log10(Id)')
